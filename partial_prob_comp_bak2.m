function prob = partial_prob_comp(x0_b,x1_b,x1,p,b_table,op_fb,M)
...
if op_fb == 1
    ...
    %% forward computation
    % interpret x1 as x0 in the script but not in comments
    % index of x0 inside the partition
    if length(x1) ~= N
        x1_i1 = sum(two_pow(x1_b).*x1);
    else
        x1_i1 = sum(two_pow(x1_b).*x1(x1_b));
    end
    ...
    %% partition
    if op_fb == 0
        for i=1: 2^N0_b % states of x1 inside the partition
            x0_bs = b_table{i,N0_b};
            prob(i) = 1;
            % fprintf('%s\n',mat2str(x0_bs))
            for k=1: N0_b % multiplication over x1 states
                x0_s = x0_bs(k);
                p_k = 0;
                for j=1: 2^N1_r % summation over the rest of x0
                    x1_i2 = x1_i2_vec(j);
                    x1_i = 1+x1_i1+x1_i2;
                    if x0_s == 1
                        p_k = p_k + p(x1_i,x0_b(k));
                    else
                        p_k = p_k + (1-p(x1_i,x0_b(k)));
                    end
                    % fprintf('p_k=%f\n',p_k);
                end
                % fprintf('k=%d x0_b(k)=%d x0_s=%d p_k=%f \n',k,x0_b(k),x0_s,p_k);
                prob(i) = prob(i)*p_k;
            end
        end
    elseif op_fb == 2  
        %% perspective
        x0_in = pick_rest(M,x1_b); % the rest of x0 in M
